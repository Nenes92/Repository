...
SPESE = {
    ...
    "ING": ["Condominio", "Garages", "Cucina", "MoneyFarm - PAC 5", "Alleanza - PAC", "World Food Programme", "Macchina", "Fastweb (Internet+Tel)", "ING C.C."],
    "BNL": ["Mutuo", "BNL C.C."],
}
...
def create_charts(stipendio_scelto, risparmiabili, df_altre_entrate):
    ...
    df_fisse.loc[(df_fisse["Categoria"] == "Netflix") | (df_fisse["Categoria"] == "Disney+") | (df_fisse["Categoria"] == "Spotify") | (df_fisse["Categoria"] == "Fastweb (Internet+Tel)") | (df_fisse["Categoria"] == "BNL C.C.") | (df_fisse["Categoria"] == "ING C.C."), "Categoria"] = "Abbonamenti"
...
def main():
    ...
    with col1:
        ...
        with col_right:
            for voce, importo in SPESE["Fisse"].items():
                if voce in ["Fastweb (Internet+Tel)"]:
                    st.markdown(
        ...
        st.markdown(f'<small style="color:#F08080; font-size: 10px;">{(spese_fisse_totali) / (stipendio_originale + sum(ALTRE_ENTRATE.values())) * 100:.2f} % dello Stipendio Totale</small> ' f'<small style="color:#B8C070; float:right; font-size: 10px;">{(risparmiabili) / (stipendio_originale + sum(ALTRE_ENTRATE.values())) * 100:.2f} % dello Stipendio Totale</small>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        for carta in ["ING", "Revolut", "BNL"]:
            ...
            if carta == "Revolut":
                ...
                st.markdown(f'Totale da &nbsp; **<em style="color: #A0A0A0;">{testo}</em> &nbsp; su <span style="color:{colore}; text-decoration: underline;">{carta}</span>:** <span style="color:{colore}">€{totale_carta:.2f}</span> <span style="font-size: 14px; color: gray;"> &nbsp;&nbsp;( + <span style="color:{colore}; font-size: 14px;">{risparmi_mese_precedente:.2f}</span> dai Risparmi + <span style="color:{colore}; font-size: 14px;">€{SPESE["Fisse"]["Disney+"]*3 + SPESE["Fisse"]["Spotify"]*5:.2f}</span> da Disn/Spoti -> Vedrai: <span style="color:{colore}; font-size: 14px;">€{somma_valori:.2f}</span> )</span>', unsafe_allow_html=True)
            else:
